...
class Clustering(_BaseAggregator):
    r"""
     A robust aggregator from paper `"On the byzantine robustness of clustered federated learning"
    ...
    def __call__(self, inputs: Union[List[BladesClient], List[torch.Tensor],
        ...
        values = torch.vstack(
            list(model for model, label in zip(updates, clustering.labels_)
                 if label == flag)).mean(dim=0)
        return values
