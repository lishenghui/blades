...
class Fltrust(object):
    r"""
         ``Fltrust`` it a trusted-based aggregator from paper
         `FLTrust: Byzantine-robust Federated Learning via Trust Bootstrapping <https://arxiv.org/abs/2012.13995>`_
        """
    def __call__(self, clients: List[BladesClient]):
        ...
        untrusted_clients = [
            client for client in clients if not client.is_trusted()
        ]
        trusted_update = trusted_client.get_update()
        trusted_norm = torch.norm(trusted_update).item()
        untrusted_updates = list(
            map(lambda w: w.get_update(), untrusted_clients))
        ...
        pseudo_gradients = torch.vstack(
            list(
                map(
                    lambda update: update * trusted_norm / torch.norm(update).
                    item(),
                    untrusted_updates,
                )))
