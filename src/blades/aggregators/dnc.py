...
class Dnc(_BaseAggregator):
    r"""
         A robust aggregator from paper `Manipulating the Byzantine: Optimizing Model Poisoning Attacks and Defenses
         for Federated Learning <https://par.nsf.gov/servlets/purl/10286354>`_
    ...
    def __call__(self, inputs: Union[List[BladesClient], List[torch.Tensor],
        ...
        for i in range(self.num_iters):
            ...
            s = np.array([(torch.dot(update - mu, v)**2).item()
                          for update in sub_updates])

            good = s.argsort()[:len(updates) -
                               int(self.fliter_frac * self.num_byzantine)]
