...
class Clippedclustering(_BaseAggregator):
    r"""
         A robust aggregator from paper `"An Experimental Study of Byzantine-Robust sAggregation Schemes in Federated
         Learning" <https://www.techrxiv.org/articles/preprint/An_Experimental_Study_of_Byzantine
    ...
    def __call__(self, inputs: Union[List[BladesClient], List[torch.Tensor],
        ...
        for idx, l2 in enumerate(l2norms):
            if l2 > threshold:
                updates[idx] = torch_utils.clip_tensor_norm_(
                    updates[idx], threshold)
        ...
        S1_idxs = list([
            idx for idx, label in enumerate(clustering.labels_)
            if label == flag
        ])
        ...
        if self.signguard:
            ...
            S2_idxs = list([
                idx for idx, label in enumerate(kmeans.labels_)
                if label == flag
            ])
