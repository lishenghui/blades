...
def _compute_scores(distances, i, n, f):
    ...
    """
    s = [distances[j][i]**2
         for j in range(i)] + [distances[i][j]**2 for j in range(i + 1, n)]
    _s = sorted(s)[:n - f - 2]
    return sum(_s)
...
def _multi_krum(distances, n, f, m):
    ...
    if m < 1 or m > n:
        raise ValueError(
            'Number of workers for aggregation should be >=1 and <= {}. Got {}.'
            .format(m, n))
    ...
    for i in range(n - 1):
        for j in range(i + 1, n):
            if distances[i][j] < 0:
                raise ValueError(
                    'The distance between node {} and {} should be non-negative: Got {}.'
...
class Multikrum(_BaseAggregator):
    r"""
      ...
      A robust aggregator from paper `"Machine Learning with Adversaries: Byzantine Tolerant Gradient Descent"
       <https://proceedings.neurips.cc/paper/2017/hash/f4b9ec30ad9f68f89b29639786cb62ef-Abstract.html>`_
    ...
    def __call__(self, inputs: Union[List[BladesClient], List[torch.Tensor]]):
        ...
        values = torch.stack([updates[i] for i in top_m_indices],
                             dim=0).mean(dim=0)
        return values
