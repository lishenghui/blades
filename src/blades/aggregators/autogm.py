...
class Autogm(_BaseAggregator):
    r"""
        A robust aggregator from paper `"Byzantine-Robust Aggregation in Federated Learning Empowered Industrial IoT"
        <https://ieeexplore.ieee.org/abstract/document/9614992>`_

        :param maxiter: Maximum number of Weiszfeld iterations. Default 100
        :param eps: Smallest allowed value of denominator, to avoid divide by zero. Equivalently, this is a smoothing
                    parameter. Default 1e-6.
    ...
    def __call__(self,
        ...
        for i in range(self.maxiter):
            ...
            for idx, local_model in enumerate(updates):
                distance[idx] = _compute_euclidean_distance(
                    local_model, median)
            ...
            for p in range(0, len(idxs)):
                eta = (sum([distance[i]
                            for i in idxs[:p + 1]]) + lamb) / (p + 1)
                if p < len(idxs) and eta - distance[idxs[p]] < 0:
                    break
                else:
                    eta_optimal = eta
            alpha = np.array(
                [max(eta_optimal - d, 0) / lamb for d in distance])
