...
rng_seed = (args.seed if
            (args.seed is not None and args.seed >= 0) else int(time.time()))
rng = random.Random(rng_seed)
if os.environ.get('LEAF_DATA_META_DIR') is not None:
    seed_fname = os.path.join(os.environ.get('LEAF_DATA_META_DIR'),
                              SEED_FILES['split'])
...
if (args.user):
    ...
    for f in files:
        ...
        if include_hierarchy:
            user_files.extend([(u, h, ns, f) for (u, h, ns) in zip(
        ...
        for i, u in enumerate(data['users']):
            ...
            if curr_num_samples >= 2:
                ...
                if args.name in ['shakespeare']:
                    ...
                    test_indices = [
                        i for i in range(num_train_samples + 80 -
                                         1, curr_num_samples)
                    ]
                else:
