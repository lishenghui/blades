...
class FangattackAdversary():
    ...
    def attack_median_and_trimmedmean(self, simulator):
        ...
        rand = torch.from_numpy(
            np.random.uniform(0, 1,
                              [len(deviation), self.num_byzantine])).type(
                                  torch.FloatTensor).to(benign_update.device)

        max_rand = torch.stack([max_range[:, 0]] * rand.shape[1]).T + \
            rand * torch.stack([max_range[:, 1] - max_range[:, 0]] * rand.shape[1]).T
        min_rand = torch.stack([min_range[:, 0]] * rand.shape[1]).T + \
            rand * torch.stack([min_range[:, 1] - min_range[:, 0]] * rand.shape[1]).T

        mal_vec = (torch.stack([(deviation < 0).type(torch.FloatTensor)] *
                               max_rand.shape[1]).T.to(device) * max_rand +
                   torch.stack([(deviation > 0).type(torch.FloatTensor)] *
                               min_rand.shape[1]).T.to(device) * min_rand).T
    ...
    def attack_multikrum(self, simulator):
        ...
        def compute_lambda(all_updates, model_re, n_attackers):
            ...
            for update in all_updates:
                ...
                distances = distance[None, :] if not len(distances) \
            ...
            max_wre_dist = torch.max(
                torch.norm((all_updates - model_re), dim=1)) / (torch.sqrt(
                    torch.Tensor([d]))[0])
    ...
    def omniscient_callback(self, simulator):
        if self.agg in ['median', 'trimmedmean']:
            ...
            raise NotImplementedError(f'Adaptive attacks to {self.agg} '
