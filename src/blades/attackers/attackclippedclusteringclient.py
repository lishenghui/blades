...
class AttackclippedclusteringAdversary():
    ...
    def chain_attack(self, simulator):
        ...
        larger_group = torch.vstack(list(
            model / torch.norm(model) for model, label in zip(benign_update, clustering.labels_) if
            label == flag)).cpu().detach().numpy()
        # theta_cross = 0
        ...
        for w in simulator.get_clients():
            if w.is_byzantine():
                if theta + theta_cross >= np.pi:
                    ...
                    mal_update = 10 * (
                            a * benign_update.mean(dim=0) / torch.norm(benign_update.mean(dim=0)) + b * mal_update)
        ...
        for w in simulator.get_clients():
            if w.is_byzantine():
                if theta + theta_cross >= np.pi:
                    ...
                    mal_update = 1000 * (a * benign_update.mean(dim=0) / torch.norm(
                        benign_update.mean(dim=0)) + b * center_point / torch.norm(center_point))
                w.save_update(mal_update)
                dis = spatial.distance.cosine(mal_update.cpu().detach().numpy(),
