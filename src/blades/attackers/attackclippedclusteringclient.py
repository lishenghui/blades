...
class AttackclippedclusteringAdversary:
        ...
        for w in simulator.get_clients():
            if w.is_byzantine():
                if theta + theta_cross >= np.pi:
                    ...
                    a = np.cos(theta + theta_cross - 1e-4) - np.sin(
                        theta + theta_cross - 1e-4
                    ) / np.tan(theta)
                    b = np.cos(theta_cross - 1e-4) + np.sin(
                        theta_cross - 1e-4
                    ) / np.tan(theta)
                    mal_update = 10 * (
                        a
                        * benign_update.mean(dim=0)
        ...
        for w in simulator.get_clients():
            if w.is_byzantine():
                if theta + theta_cross >= np.pi:
                    ...
                    a = np.cos(theta + theta_cross) - np.sin(
                        theta + theta_cross
                    ) / np.tan(theta)
    ...
    def omniscient_callback(self, simulator):
        if self.linkage == "single":
            ...
            raise NotImplementedError(f"linkage {self.linkage} " f"is not implemented.")
