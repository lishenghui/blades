...
class AttackclippedclusteringAdversary():
    ...
    def chain_attack(self, simulator):
        ...
        for i in range(num):
            for j in range(num):
                if i == j:
                    ...
                    dis_max[i, j] = spatial.distance.cosine(
                        np_models[i, :], np_models[j, :])
        ...
        for idx_i, label_i in enumerate(clustering.labels_):
            for idx_j, label_j in enumerate(clustering.labels_):
                if idx_j == idx_i:
                    continue
                dis = spatial.distance.cosine(np_models[idx_i, :],
                                              np_models[idx_j, :])
        ...
        larger_group = torch.vstack(
            list(model / torch.norm(model)
                 for model, label in zip(benign_update, clustering.labels_)
                 if label == flag)).cpu().detach().numpy()
        # theta_cross = 0
        dis2mean = [
            spatial.distance.cosine(benign_update, benign_mean)
            for benign_update in larger_group
        ]
        idx_max_dis = np.argmax(dis2mean)
        theta = np.arccos(1 - dis2mean[idx_max_dis])
        mal_update = benign_update[idx_max_dis] / torch.norm(
            benign_update[idx_max_dis])
        print(dis_cross)
        for w in simulator.get_clients():
            if w.is_byzantine():
                if theta + theta_cross >= np.pi:
                    ...
                    a = (np.cos(theta + theta_cross - 1e-4) -
                         np.sin(theta + theta_cross - 1e-4) / np.tan(theta))
                    b = (np.cos(theta_cross - 1e-4) +
                         np.sin(theta_cross - 1e-4) / np.tan(theta))
                    mal_update = 10 * (a * benign_update.mean(dim=0) /
                                       torch.norm(benign_update.mean(dim=0)) +
                                       b * mal_update)
                    theta = theta + theta_cross - 1e-4
                w.save_update(mal_update)
                dis = spatial.distance.cosine(
                    mal_update.cpu().detach().numpy(),
                    benign_update.mean(dim=0).cpu().detach().numpy())
                print(dis)
        return
    ...
    def attack_average(self, simulator):
        ...
        for i in range(len(clustering.labels_)):
            for j in range(i + 1, len(clustering.labels_)):
                if clustering.labels_[i] != clustering.labels_[j]:
                    dis = spatial.distance.cosine(np_models[i, :],
                                                  np_models[j, :])
                    dis_cross += dis
                    dis_size += 1
        ...
        theta = np.arccos(1 -
                          spatial.distance.cosine(center_point, benign_mean))
        print(dis_cross)
        for w in simulator.get_clients():
            if w.is_byzantine():
                if theta + theta_cross >= np.pi:
                    ...
                    a = (np.cos(theta + theta_cross) -
                         np.sin(theta + theta_cross) / np.tan(theta))
                    b = (np.cos(theta_cross) +
                         np.sin(theta_cross) / np.tan(theta))
                    mal_update = 1000 * (
                        a * benign_update.mean(dim=0) /
                        torch.norm(benign_update.mean(dim=0)) +
                        b * center_point / torch.norm(center_point))
                w.save_update(mal_update)
                dis = spatial.distance.cosine(
                    mal_update.cpu().detach().numpy(),
    ...
    def omniscient_callback(self, simulator):
        if self.linkage == 'single':
            ...
            raise NotImplementedError(f'linkage {self.linkage} '
