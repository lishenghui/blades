...
class BaseDataset(ABC):
    ...
    def generate_datasets(
        self, path="./data", iid=True, alpha=0.1, num_clients=20, seed=1
    ):
        pass
    ...
    def get_dls(self):
        ...
        with open(self._data_path, "rb") as f:
            (_, train_clients, train_data, test_clients, test_data) = [
        ...
        for idx, u_id in enumerate(train_clients):
            train_dls.append(
                self._preprocess_train_data(
                    data=np.array(train_data[u_id]["x"]),
            ...
            test_dls.append(
                self._preprocess_test_data(
                    data=np.array(test_data[u_id]["x"]),
