...
class FLDataset(object):
    ...
    Args:
        ...
        test_dataloaders (list, optional): dataloaders of test data for input.
            Two lists of dataloders should be in corresponding order to the
    ...
    def __init__(
        ...
        if not test_dataloaders:
            warn(
                "No test data is given. Model evaluation will be based on train data. "
            )
            test_dataloaders = train_dataloaders
        if len(train_dataloaders) != len(test_dataloaders):
            raise Exception(
                "Invalid Input: Numbers of train dataloaders and test dataloaders "
                "should be equal. "
            )
        self._train_dls = {}
        self._test_dls = {}
        for idx, (traindl, testdl) in enumerate(
            zip(train_dataloaders, test_dataloaders)
        ):
