...
def parse_arguments():
    ...
    agg_args = {
        'trimmedmean': {
            'num_excluded': options.num_byzantine
        },
        'median': {},
        'mean': {},
        'signguard': {},
    ...
    options.log_dir = (
        EXP_DIR + f'_{options.algorithm}' + f'/b{options.num_byzantine}' +
        f'_{options.attack}' +
        ('_' + '_'.join([k + str(v) for k, v in options.attack_kws.items()])
         if options.attack_kws else '') + f'_{options.agg}' +
        ('_' +
         '_'.join([k + str(v) for k, v in options.aggregator_kws.items()])
         if options.aggregator_kws else '') + (f'_lr{options.lr}') +
        (f'_serv_momentum{options.serv_momentum}') +
        (f'_bz{options.batch_size}') + (f'_localround{options.local_round}') +
        ('_noniid' if options.non_iid else '') +
        (f'_privacy_epsilon{options.dp_privacy_epsilon}_clip_threshold'
         f'{options.dp_clip_threshold}' if options.dp else '') +
