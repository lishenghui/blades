...
cache_name = options.dataset + '_' + options.algorithm + ('_noniid' if not options.non_iid else '') + \
...
privacy_factor = args.dp_privacy_sensitivity * math.sqrt(
    2 * math.log(1.25 / args.dp_privacy_delta)) / args.dp_privacy_epsilon
...
conf_args = {
    ...
    if options.attack in options.adversary_args else {},
...
if options.algorithm == 'fedsgd':
    opt = torch.optim.SGD(model.parameters(),
                          lr=0.1,
                          momentum=options.serv_momentum)
    lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(
        opt, milestones=[2000, 3000, 5000], gamma=0.5)
    assert options.local_round == 1,\
        'fedsgd requires that only one SGD is taken.'
    ...
    lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(
        opt,
        milestones=[
            options.global_round / 3, options.global_round / 2,
