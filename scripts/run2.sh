...
run_all_aggs() {
    # for attack in "signflipping" #"alie" "ipm" "noise" "labelflipping" 
    # do
    ...
    #     done
    # done
    for attack in "ipm" #"alie" "ipm" "noise" "labelflipping" 
    do
        for serv_momentum in 0.9  #1 3 5 7 9
        do
            args="--dataset $1 --algorithm $2 --global_round $3 --local_round $4  --agg $5 $6 --num_gpus 4 --num_byzantine 0 --use-cuda --batch_size 64 --seed 0 --serv_momentum $serv_momentum --attack $attack"
            echo ${args}
            nohup python main.py ${args} &
        done
    done
    ...
    do
        for serv_momentum in 0.9  #1 3 5 7 9
        do
            args="--dataset $1 --algorithm $2 --global_round $3 --ipmlarge --local_round $4  --agg $5 $6 --num_gpus 4 --num_byzantine 5 --use-cuda --batch_size 64 --seed 0 --serv_momentum $serv_momentum --attack $attack"
            echo ${args}
            nohup python main.py ${args} &
        done
    done
...
for agg in 'signguard' #'median' 'trimmedmean' 'centeredclipping' 'mean' 'krum' 'clustering' 'geomed' 'autogm'
do 
    nohup bash -c "run_all_aggs $dataset fedavg 600 50 $agg" &
